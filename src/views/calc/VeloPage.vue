<template>
    <div class="tidy">
        ...
        <p>
            Wind can only help going one way. Consider these variables...
            <br />
            distance {{ distance }} / speed {{ topSpeed }} <br />
            {{ distance / topSpeed }} hours (one way)
        </p>

        <h2 class="full">
            Best travel time:
            <b>{{ timeNormal.toLocaleString() }}</b
            >&nbsp;hours
        </h2>
        ...
        <h2 class="full">
            Adjusted: <b>{{ timeTotal.toLocaleString() }}</b
            >&nbsp;hours
        </h2>
        ...
        <h3 class="full">
            Increased by: <b>{{ (timeTotal - timeNormal).toLocaleString() }}</b
            >&nbsp;hours
        </h3>
        <p>
            <span v-if="!played"> Try a wind speed close to the top speed! <br /> </span
            ><span v-else>
