<template>
    ...
    <div class="tidy">
        <h2>Range (tuple)</h2>
        <p>A range tuple is a value pair representing a MIN and MAX.</p>
        <p>Strictly, the minimum can be any number smaller than the maximum.</p>
        <div class="flex lined">
            <div>
                <div>
                    <p>Set spread...</p>
                    <label> min <input v-model.number="spread.min" type="number" /> </label>
                    <label> max <input v-model.number="spread.max" type="number" /> </label>
                    <br />
                    <label> strict <input v-model.number="spread.strict" type="checkbox" /> </label>
                </div>
            </div>
            <div>
                <h3>Properties:</h3>
                ...
                <hr />
                <p :class="{ error: !spread.valid }"><b>valid:</b> {{ spread.valid }}</p>
        ...
        <div class="flex lined">
            <div>
                <p>Set weight...</p>
                <label> abs <input v-model.number="weight.abs" type="number" /> </label>
                <label> rel <input v-model.number="weight.rel" type="number" step="0.1" /> </label>
                <label>
                    srel (signed&nbsp;normal)
            ...
            <div>
                ...
                <hr />
                <p :class="{ error: !weight.bounded }"><b>bounded:</b> {{ weight.bounded }}</p>
        ...
        <p>
            ...
            <br />
            An absolute value (abs: {{ weight.abs.toLocaleString() }}) also has a relative value
            (rel: {{ weight.rel.toLocaleString() }}) to that range.
        </p>
        <p>
            <i>
